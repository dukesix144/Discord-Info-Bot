...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('sandstormleaderboard')
        .setDescription('Top 10 Sandstorm Leaderboard @ ↳ЯR↰'),
    async execute(interaction) {
        try {
            const response = await axios.get('https://www.rrgaming.net/api/leaderboard_sandstorm_slash.php');
            ...
            const embed = new EmbedBuilder()
                .setColor(0x0099FF)
                .setTitle('Top 10 Sandstorm Leaderboard @ ↳ЯR↰')
            ...
            embed.addFields(
                ...
                {
                    name: 'Player',
                    value: leaderboard.slice(0, 10).map(entry => `${entry.player_name}`).join('\n') || 'N/A',
                    inline: true
                }
            );
